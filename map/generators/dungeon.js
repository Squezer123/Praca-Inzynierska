...
export default class Dungeon extends Generator {
    ...
    new_room(key) {
        // Ustaw domyślny klucz, jeśli podany klucz nie istnieje
        key = key || this.random.choose(this.room_tags, false);

        // Sprawdź, czy konfiguracja dla danego klucza istnieje
        const opts = this.options.rooms[key];
        if (!opts) {
            console.error(`Invalid room key: ${key}`);
            return null;
        }

        // Utwórz nowy pokój na podstawie opcji
        const room = new Room({
        ...
        // Usuń użyty klucz z `room_tags`
        const index = this.room_tags.indexOf(key);
        if (index !== -1) {
            this.room_tags.splice(index, 1);
        }

        // Jeśli to pierwszy pokój, ustaw go jako `initial_room`
        if (key === 'initial') {
